...
void loop() {
  ...
  if (radio.available()) {
    ...
    radio.read(receive_buffer, sizeof(receive_buffer)); // Đọc dữ liệu vào receive_buffer
    receive_start_detect++; // Tăng biến đếm để kiểm tra dữ liệu đầy đủ

    for (int i = 0; i < sizeof(receive_buffer); i++) {
      Serial.print(receive_buffer[i], HEX); // In từng byte dưới dạng hex
      Serial.print(" ");
    }
    Serial.println(); // Dòng mới sau khi in xong
    Serial.println("Radio *****availalbel****");

    if (receive_start_detect >= 2) get_data(); // Nếu có đủ dữ liệu, xử lý dữ liệu
  } else {
