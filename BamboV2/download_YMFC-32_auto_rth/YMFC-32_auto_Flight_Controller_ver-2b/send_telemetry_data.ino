...
void send_telemetry_data(void) {
  Serial.println("nv___send_telemetry_data__telemetry_loop_counter: ");
  ...
  // Send the telemetry_send_byte via NRF24L01
  Serial.println("__________start sending datas_____");
  if (telemetry_loop_counter <= 34) {
      check_byte ^= telemetry_send_byte;
      if (radio.write(&telemetry_send_byte, sizeof(telemetry_send_byte))) {
        Serial.println("Data sent successfully");
        Serial.println(telemetry_loop_counter);
      } else {
          Serial.println("Failed to send data");
      }
      // radio.write(&telemetry_send_byte, sizeof(telemetry_send_byte));
  }
}
