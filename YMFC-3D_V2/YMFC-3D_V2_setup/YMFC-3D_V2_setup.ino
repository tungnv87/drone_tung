...
void loop() {
  ...
  if (error == 0) {
    delay(2000);
    Serial.println(F("Place all sticks and subtrims in the center position within 10 seconds. - stick trái - từ dưới lên  "));
    for (int i = 9; i > 0; i--) {
      delay(1000);
      Serial.print(i);
      Serial.print(" ");
  ...
  if (error == 0) {
    Serial.println(F("Move the throttle stick to full throttle and back to center - stick trái - move lên trên rồi về giữa"));
    ...
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Move the roll stick to simulate left wing up and back to center - stick phải - move phải"));
    //Check for throttle movement
  ...
  if (error == 0) {
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Move the pitch stick to simulate nose up and back to center - stick phải - move xuống"));
    //Check for throttle movement
  ...
  if (error == 0) {
    Serial.println(F(""));
    Serial.println(F(""));
    Serial.println(F("Move the yaw stick to simulate nose right and back to center - stick trái - move phải"));
    //Check for throttle movement
  ...
  if (error == 0) {
    ...
    Serial.println(F("Lift the left side of the quadcopter to a 45 degree angle within 10 seconds - nhấc bên trái 45°"));
    //Check axis movement
    check_gyro_axes(1);
    if (error == 0) {
      Serial.println(F("OK!"));
      ...
      Serial.println(F(""));
      Serial.println(F(""));
      Serial.println(F("Lift the nose of the quadcopter to a 45 degree angle within 10 seconds - nhấc đầu 45°"));
      //Check axis movement
      check_gyro_axes(2);
    }
    if (error == 0) {
      ...
      Serial.println(F(""));
      Serial.println(F(""));
      Serial.println(F("Rotate the nose of the quadcopter 45 degree to the right within 10 seconds - xoay phải 45°"));
      //Check axis movement
      check_gyro_axes(3);
    }
    if (error == 0) {
...
//    Serial.print(F("Wait stick zero: "));// Serial.println(F(wait_sticks_zero));
...
void register_min_max() {
  ...
  Serial.println(F("**register_min_max11"));
